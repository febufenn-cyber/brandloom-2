...
const required = [
  ...
  'ANTHROPIC_API_KEY', 'ANTHROPIC_MODEL', 'TOKEN_ENCRYPTION_KEY', 'CRON_SECRET',
  'WEB_ORIGIN', 'PUBLIC_API_ORIGIN', 'META_APP_ID', 'META_APP_SECRET',
...
const httpsKeys = ['SUPABASE_URL', 'WEB_ORIGIN', 'PUBLIC_API_ORIGIN', 'META_REDIRECT_URI', 'STRIPE_SUCCESS_URL', 'STRIPE_CANCEL_URL'];
...
if (process.env.EXPECTED_MIGRATION_VERSION && process.env.EXPECTED_MIGRATION_VERSION !== latestMigration) {
  errors.push(`EXPECTED_MIGRATION_VERSION must equal ${latestMigration}.`);
}
